...
# Extract factor matrices (Z) for Hungarian alignment
...
ggplot(var_merged, aes(x = R2_ref, y = R2_scot, color = view)) +
  geom_point(size = 3) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  facet_wrap(~view) +
  theme_bw() +
  labs(
    x = "Reference variance explained",
    y = "SCOT+ variance explained",
